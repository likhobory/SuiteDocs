...
After the installer completes, you need to configure two background processes to keep SuiteCRM running correctly.

=== 4.1 Cron Job (Schedulers)
...
See the link:../../administration-panel/schedulers[Schedulers] page for full instructions on setting up the cron job.

=== 4.2 Messenger Worker (Async Tasks)

Starting from SuiteCRM 8.10, background tasks (such as manual migration tasks) are processed by the Symfony Messenger worker. Without a running worker, these tasks remain in "Pending" status indefinitely.

See the link:../../async-tasks/messenger-setup[Messenger Setup] guide for instructions on configuring the worker for your environment.
