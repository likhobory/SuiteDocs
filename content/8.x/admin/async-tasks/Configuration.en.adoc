...
Since SuiteCRM uses Symfony Messenger, alternative transports such as AMQP (RabbitMQ), Redis, and others are also available. For details on all supported transports, see the https://symfony.com/doc/current/messenger.html#transport-configuration[Symfony Messenger transport documentation].

[cols="15,45,40"]
|===
| *Type* | *DSN Format* | *Notes*

| Doctrine | `doctrine://default?queue_name=<name>` | *Default.* Uses your existing database. No additional infrastructure required.
