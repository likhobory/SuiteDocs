...
{{% notice warning %}}
The worker *must* run as the same system user that runs your web server (e.g. `www-data`). If the worker runs as a different user, file operations (uploads, media object storage) will fail with permission errors.
{{% /notice %}}

If you are using Docker, run the worker inside the container:

[source,bash]
----
docker exec -u www-data <container-name> php bin/console messenger:consume internal-async --time-limit=3600 --memory-limit=256M
----

Replace `<container-name>` with the name of your PHP container.
...
[source,bash]
----
* * * * * www-data php /path/to/suitecrm/bin/console messenger:consume internal-async --time-limit=55 --memory-limit=256M > /dev/null 2>&1
----
